...
class VAE():
    ...
    def __init__(self, architecture=[], d_hyperparams={}, meta_graph=None,
                 save_graph_def=True, data_path=None,log_dir="./log"):
    ...
    def _buildGraph(self):
        ...
        x_in=tf.placeholder_with_default(x_q_in_norm,shape=(None,1024),name="x_in")
    ...
    def train(self, max_iter=np.inf, 
        ...
        tf.train.start_queue_runners(sess=self.sesh)
        ...
        try:
            ...
            while True:
                #x, _ = X.next_batch(self.batch_size)
                feed_dict = {self.dropout_: self.dropout}
                fetches = [self.x_in, self.cost, self.global_step, self.train_op]
                
                # Benchmark the learning
                run_options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE) 
                run_metadata = tf.RunMetadata()
                
                x_in, cost,i, _ = self.sesh.run(fetches, feed_dict,
                                                options=run_options,
                                                run_metadata=run_metadata)
                
                # get runtime statistics every 100 runs
                if i%100 == 0:
                ...
                """
                if i%5000 == 0 and verbose:# and i >= 10000:
                ...
                if i%5000 == 0 and save:
                    interfile=os.path.join(os.path.abspath(outdir), "{}_vae_{}".format(
                            self.datetime, "_".join(map(str, self.architecture))))
                    saver.save(self.sesh, interfile, global_step=self.step)
                    
                if i >= max_iter:
                    print("final avg cost (@ step {} = ): {}".format(
                        i, err_train / i))
                    now = datetime.now().isoformat()[11:]
                    print("------- Training end: {} -------\n".format(now))
